/*
  ...
  row in our image, with a length equal to our canvas’ X dimension. You are given a canvas (2 dimensional array of integers),
  starting coordinate (2-element array), and the color to flood (integer value). Build floodFill(canvas2D,startXY,newColor)
  ! Replace a pixel’s color value only if it is the same color as the origin coordinate and is directly adjacent via X or Y
  to another pixel you will change. Note: diagonally related pixels are not considered adjacent.
  Input:
  [
    [3, 2, 3, 4, 3],
    [2, 3, 3, 4, 0],
  ...
  and repeat the process with its directly-adjacent neighbor cells. We repeat this until th e entire zone
  of similarly-colored cells is changed .
  Output:
  [
    [3, 2, 1, 4, 3],
    [2, 1, 1, 4, 0],
...
//     console.log(`X is ${x}`)
//     console.log(`Y is ${y}`)
//     let startValue = canvas2D[y][x];
//     canvas2D[y][x] = newColor;
//     if (y == 0 || y == canvas2D.length - 1) {
//         // check if y is out of bounds
//         return canvas2D
//     } else if (x == 0 || x == canvas2D[x].length - 1) {
//         // check if x is out of bounds
//         return canvas2D
//     } else {
//         console.log(startValue)
//         let newStart = []
//         if (canvas2D[y][x - 1] == startValue) {
//             newStart.push(x - 1)
//             newStart.push(y)
//             floodFill(canvas2D, newStart, newColor)
//         }
//         if (canvas2D[y][x + 1] == startValue) {
//             newStart.push(x + 1)
//             newStart.push(y)
